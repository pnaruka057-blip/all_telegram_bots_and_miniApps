...
const updateShowWizard = new Scenes.WizardScene(
    ...
    async (ctx) => {
        ...
        if (text) {
            if (text === CANCEL) return cancelWizard(ctx, "_edit_show");
            if (text === BACK) return scense_stepBack(ctx, 4, "🎭 Enter the *Genre* again:", "_edit_show");
    ...
    async (ctx) => {
        const text = ctx.message.text;
        if (text === CANCEL) return cancelWizard(ctx, "_edit_show");
        if (text === BACK) return scense_stepBack(ctx, 5, "🖼️ Send the *Thumbnail URL* again:", "_edit_show");
    ...
    async (ctx) => {
        const text = ctx.message.text;
        if (text === CANCEL) return cancelWizard(ctx, "_edit_show");
        if (text === BACK) return scense_stepBack(ctx, 6, "🔗 Enter *Download Links* again:", "_edit_show");
